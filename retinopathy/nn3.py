...
def load_train_data(datafile, datadir):
    ...
    for j in range(1, i):
        ...
        img = cv2.imread(datadir + "/" + filename + ".jpeg", cv2.CV_LOAD_IMAGE_GRAYSCALE)
        img_r = cv2.resize(img, (150, 150))
#        print "IMG_R Shape: {}".format(img_r.shape)
        if j == 1:
            X = np.zeros((i-1, 1, img_r.shape[0], img_r.shape[1]))
            labels = np.empty((i-1), dtype=np.int32)
        img_r = img_r.reshape(1, 150, 150)
        X[j-1] = np.vstack(img_r) / 255
